...
def init():
    man="""
    Script for analyzing typical ground and excited state Gaussian16A03-calulations.
    The inputs are automatically checked for valid g16-files.
    By default, the output is printed to the terminal.
    Extracted information is:
        - computed time or frequency
        - nr. of imaginary frequencies (for freq calc.)
        - total ground or excited state (from root) energy. This is E+ZPE for freq calcs!
        - zero-point energy correction, ZPE (for freq calc.)
        ES-information: (printed for root, or for all roots with -f flag.)
        - optimized root.
        - vertical energy of this root.
        - Oscillator strength.
        - orbital transitions and percentages (2*coef**2).
    --force is used by default if 'fc' is detected in filename (unset with -g).
    """
    parser = ArgumentParser(formatter_class=RawDescriptionHelpFormatter, epilog=man)
    parser.add_argument("dirname", nargs='+', help="Give list of folders and/or files for analysis")
    parser.add_argument("-w", help="Write output to csv-file instead of stdout", action="store_true")
    parser.add_argument("-o", "--outfile", help="Give name of csv-file [results.csv]")
    parser.add_argument("-v", "--verbose", help="Output to stdout, even with -w or -o", action="store_true")
    parser.add_argument("-f", "--force", help="Force analysis of all ES's e.g. vertical transition, franck-condon values (vt, fc).", action="store_true")
    ...
    ### General structure:                                             ###
    ### for each file:                                                 ###
    ###     calls main --> writes results to csv                       ###
    ### print information from csv-file                                ###
    ###                                                                ###
    ### main: - differs between 1 or more roots: log1out and logfcout  ###
    ###       - calls startana                                         ###
    ###       - writes to csv-file                                     ###
    ###                                                                ###
    ### startana: - gets general information (time etc.)               ###
    ###           - calls gsana or esana for ground or excited state   ###
    ######################################################################
    ...
    if args.ground and args.force:
        print('force and ground not compatible, using default', file=sys.stderr)
    elif args.force:
        force=1
    elif args.ground:
        force=-1
    ...
    if csvname.is_file():
        yn = input(f"{csvname} already exist, overwrite? [y/n] ")  # gets written to stderr, could be a problem
        if 'y' in yn:
            csvname.unlink()
        else:
            exit(0)
    ...
    for dirdir in args.dirname:
        ...
        if drg.is_file():
            ...
            print(f'\"{dirdir}\" is no file or dir?', file=sys.stderr)
            continue
    ...
    if csvname.is_file():
        with open(csvname, 'r+') as csvfile:
            ...
            csvfile.seek(0, 0)
            csvfile.write('name, debug, time, imag, gse/ese, zpe, esopt,emmE-eV, f, ifweights\n' + content)
        if args.verbose or (not args.w and not args.outfile):
            print()
            with open(csvname, 'r') as csvfile:
                if args.space or args.readable:
                    ...
                    ## this part is written kinda bad... --> better to use pandas probably
                    for line in csvfile.readlines():
                        ...
                        ifcoefs = [', '.join(ll[i + 9:i + 12]) for i in range(0, len(ll[9:]), 3) if
                                   float(ll[i + 11]) > 0.2]
                        print(f"{fun(ll[0], namelen)}, {fun(ll[2], timelen)}, {fun(ll[3], 5)},"  # name, time, imag
                              f"{fun(ll[4], 14)}, {fun(ll[5], 8)}, {fun(ll[6], 6)}, {fun(ll[7], 7)}, {fun(ll[8], 6)}, "  # e, zpe, esopt, emmE, f
                              f"{', '.join(ifcoefs)}")
                else:
        ...
        print('no valid gaussian logfiles selected', file=sys.stderr)
        exit(1)
...
def main(filename, outname, fc=0, t_unit=False, eff=False):
    ...
    def logFCout(infile, outfile):
        ...
        try:
            ...
            print('   --> Warning: no es-properties. Skipping ' + name, file=sys.stderr)
            return
        if esp is None:
            print('   --> Warning: no es-properties. Skipping ' + name, file=sys.stderr)
        outfile.writerow(
    ...
    with open(totfile, "r") as fl:
        ...
        for line in fl.readlines():
            ...
            if re.search('Error termination', line):
                er_flag = True
    if er_flag:
        print('    --> Warning: terminated with error, values are probably wrong!', file=sys.stderr)
    if of_flag:
        if oc_flag < el_flag:
            print('    --> Warning: opt not converged', file=sys.stderr)
        if es_flag:
...
def es_ana(totfile, dic, es_opt, es_max, linenums, of=True):
    ...
    if ediff1 != 0 or ediff2 != 0:
        if of:
            print('Calc not finished?\n   --> Ediff = %f\n   --> Ediff2 = %f' % (ediff1, ediff2), file=sys.stderr)
        else:
            ese_sum = tde
            zpe_corr = 'No Freq!'
